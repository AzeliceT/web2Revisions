...
router.get("/", function (req, res, next) {
  ...
  const catalogueFiltred = catalogue.filter(
    (film) => film.duration >= minimumDurationParsed
  );
...
router.post("/", (req, res) => {
  ...
  const filmExistant = catalogue.find(
  ...
  const lastItemIndex =
    catalogue?.length !== 0 ? catalogue.length - 1 : undefined;
...
router.delete("/:id", (req, res) => {
  ...
  const indexDuFilm = catalogue.findIndex(
    (film) => film.id == idFilmASupprimer
  );
  ...
  const tableauElementSupprimer = catalogue.splice(indexDuFilm, 1);

  console.log("tableauElementSupprimer ", tableauElementSupprimer[0]);
...
router.patch("/:id", (req, res) => {
  ...
  //On mets dans la constante idFilmAModifie l'id qu'on récupère dans REST client/films.http
  const indexDuFilm = catalogue.findIndex((film) => film.id == idFilmAModifie);
  //Si cet id existe, on mets son index dans la constante indexDuFilm

  if (indexDuFilm < 0) {
    return res.sendStatus(404);
  }
  //Si le film existe pas on envoie un Status

  const title = req?.body?.title;
  //Cette ligne de code affecte à la variable title la valeur de req.body.title seulement si req, req.body, et req.body.title ne sont pas null ou undefined. 
  //Si l'une de ces valeurs est null ou undefined, title prendra la valeur undefined sans générer d'erreur 
  ...
  if (
    ...
    //Si le titre est défini MAIS qu'il y a des espaces '    ' alors on renvoi le Status
    (duration !== undefined &&
  ...
  const filmExistant = catalogue[indexDuFilm];
  //On mets la valeur des champs du films dans la constante filmExistant

  const attributsFilmAChanger = req.body;
  //Dans la constante attributsFilmAChanger on va mettre les nouvelles valeurs qu'on va 
  //récupérer dans REST client/films.http

  const filmAModifie = {
    ...filmExistant,
    ...attributsFilmAChanger,
    //Dans la constante filmAModifie on va y remplacer l'ancienne valeur du filmExistant par 
    //les nouvelles valeurs.
  };

  catalogue[indexDuFilm] = filmAModifie;
  //On oubli pas de changer ce film par les nouvelles valeurs dans le tableau catalogue
  return res.json(filmAModifie);
  //On renvoi le tableau sous format json
});
...
router.put("/:id", (req, res) => {
  ...
  const attributsFilmAChanger = req.body;
    ...
    catalogue[indexDuFilm]=filmEntierModifie;
